...
'''
def register(data): 
    return driver.add_user(name=data['name'], date_of_birth=data['dob'], username=data['username'], password=data['password'])
...
def get_transactions(user, offset, limit): 
    return driver.get_transactions(user, offset, limit)

'''
function to add a transaction record for the user
'''
def add_transaction(user, data): 
    return driver.add_transaction(user, date=data['date'], type=data['type'], category=data['category'], dealer=data['dealer'], status=data['status'])

'''
function to delete a transaction record for a user given the id
'''
def delete_transaction(user, doc_id): 
    return driver.delete_transaction(user, doc_id)

'''
function to get a specific transaction record given the id
'''
def get_transaction_by_id(user, doc_id): 
    return driver.get_tansaction_by_id(user, doc_id)

'''
function to update the transaction given the new data
'''
def update_transactions(user, data): 
    return driver.update_transactions(user, doc_id=data['id'], date=data['date'], type=data['type'], category=data['category'], dealer=data['dealer'], status=data['status'])
...
def get_debt_list(user, offset, limit): 
    return driver.get_debt_list(user, offset, limit)
...
def get_owe_list(user, offset, limit): 
    return driver.get_owe_list(user, offset, limit)
...
def get_credit_transactions(user, offset, limit): 
    return driver.get_credit_transactions(user, offset, limit)
...
def get_debit_transactions(user, offset, limit): 
    return driver.get_debit_transactions(user, offset, limit)
...
def get_saved_transactions(user, offset, limit): 
    return driver.get_saved_transactions(user, offset, limit)